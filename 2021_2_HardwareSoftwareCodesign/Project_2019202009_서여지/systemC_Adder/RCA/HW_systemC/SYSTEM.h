...
SC_MODULE(SYSTEM) {
	...
	SC_CTOR(SYSTEM) :clk_sig("clk_sig", 1, SC_NS) {
		rca32_0 = new rca32("rca32_0");		//RCA
		rca32_0->clk(clk_sig);
		
		rca32_0->A(A_sig);
		rca32_0->A_valid(A_valid_sig);
		rca32_0->A_ready(A_ready_sig);

		rca32_0->B(B_sig);
		rca32_0->B_valid(B_valid_sig);
		rca32_0->B_ready(B_ready_sig);

		rca32_0->Cin(Cin_sig);
		rca32_0->Cin_valid(Cin_valid_sig);
		rca32_0->Cin_ready(Cin_ready_sig);

		rca32_0->result(result_sig);
		rca32_0->result_valid(result_valid_sig);
		rca32_0->result_ready(result_ready_sig);

		rca32_0->Cout(Cout_sig);
		rca32_0->Cout_valid(Cout_valid_sig);
		rca32_0->Cout_ready(Cout_ready_sig);
