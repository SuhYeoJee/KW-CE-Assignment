...
void csa32::csa32_main() {
	...
	A_ready.write(1); // input ready 세팅
	do {
		wait();
	}while (!A_valid.read());// input valid 확인
	a = A.read(); // input 읽기
	A_ready.write(0); // input ready 세팅


	B_ready.write(1); // input ready 세팅
	do {
		wait();
	} while (!B_valid.read());// input valid 확인
	b = B.read(); // input 읽기
	B_ready.write(0); // input ready 세팅

	Cin_ready.write(1); // input ready 세팅
	do {
		wait();
	} while (!Cin_valid.read());// input valid 확인
	CIN = Cin.read(); // input 읽기
	Cin_ready.write(0); // input ready 세팅
