...
floating_point
	ldr r1, [r0]		;read hexadeximal number
	cmp r1, #0			;check 'end'
	BXEQ lr				;end
	
	mov r10, #1			;sign bit			
	lsl r10, #31		;1000_0000_0000...
	MOV r11, #0xFF		;Exponent bit is 8bit
	LSL r11, #23		;0111_1111_1000...
	MOV r12, #0x7F		;Mantissa bit is 23bit
	LSL r12, #8			;1000_0000_0111...
	ADD r12, r12, #0xFF
	LSL r12, #8
	ADD r12, r12, #0xFF
	
	;find exponent, mantissa
	;find first '1' (after sign bit)
	mov r2, r10
	MOV r3, #158
	and r5, r10, r1	;get sign bit
loop_first1
	lsr r2, #1	;0100_00.. -> 0010_00... -> 0001_00..
	subs r3, r3, #1
	BEQ endloop_first1 ;if zero, break
	ANDS r4, r1, r2
	BEQ loop_first1
	
endloop_first1
	;r2 is first '1' after this '1': mantissa
	;r3 is exponent
	lsl r1, r1, #1
	lsls r2, r2, #1
	BNE endloop_first1
	...
	lsr r1, r1, #9		;mantissa -> lower 23 bit
	lsl r3, r3, #23 	;exponent -> front of the mantissa
	add r1, r1, r3		;add exponent bit
	add r1, r1, r5		;add sign bit
	
	str r1, [r0], #4 	;save floatingpoint number
	B floating_point
...
insertion_sort
	mov r6, r0 ; 'end' address
	ldr r0, =dataset ; r0 = address of a element to sort
i_loop_sort
	add r0, r0, #4	;start at second element, next element
	ldr r2, [r0] ;data to sort
	cmp r2, #0			;check 'end'
	BXEQ lr				;end
	ldr r1, =dataset ; fisrt element
i_loop_cmp
	ldr r3, [r1] ;data to cmp
	; when r3 = 0('end'), must sort

	;1. sign bit cmp
	and r4, r2, r10 ; r2's sign bit
	and r5, r3, r10 ; r3's sign bit
	cmp r4, r5		;cmp sign bit (0 or 1)
	BHI i_if_grater_r3 ; 10 1-0 = 1
	BMI i_if_grater_r2 ; 01 0-1 = -1
	
	cmp r4, r10	;11 or 00
	BEQ i_cmp_neg	; cmp negative number
i_cmp_pos
	;2. exponent cmp
	and r4, r2, r11 ; r2's exponent bit
	and r5, r3, r11 ; r3's exponent bit
	cmp r4, r5		;cmp exponent bit
	BHI i_if_grater_r2 ; r4 > r5
	BMI i_if_grater_r3 ; r5 > r4
	
	;3. mantissa cmp
	and r4, r2, r12 ; r2's mantissa bit
	and r5, r3, r12 ; r3's mantissa bit
	cmp r4, r5		;cmp mantissa bit
	BHI i_if_grater_r2 ; r4 > r5
	BLS i_if_grater_r3 ; r5 >= r4
...
i_cmp_neg
	...
	and r5, r3, r11 ; r3's exponent bit
	cmp r4, r5		;cmp exponent bit
	BHI i_if_grater_r3 ; |r4| > |r5|
	BMI i_if_grater_r2 ; |r5| > |r4|
	...
	cmp r4, r5		;cmp mantissa bit
	BHI i_if_grater_r3 ; |r4| > |r5|
	BLS i_if_grater_r2 ; |r5| >= |r4|
	
i_if_grater_r2
	add r1, r1, #4 ;next address
	b i_loop_cmp ;next cmp

i_if_grater_r3 ;or same
	;sort
	cmp r0, r1		 ;to check sort end
	BCC i_loop_sort	 ;next element sort
	swp r2, r2, [r1] ;r2 <-> r3
	add r1, r1, #4
	B i_if_grater_r3
...
bubble_sort
	...
	ldr r8, =dataset
b_loop_sort
	ldr r0, =dataset ; start at first
b_loop_cmp
	add r1, r0, #4 	;element to cmp
	cmp r1, r7		;check end
	BEQ b_end_cmp		;end
	ldr r2, [r0]	; r0 data
	ldr r3, [r1]	; r1 data
	...
	and r5, r3, r10 ; r3's sign bit
	cmp r4, r5		;cmp sign bit (0 or 1)
	BHI b_if_grater_r3 ; 10 1-0 = 1
	BMI b_if_grater_r2 ; 01 0-1 = -1
	
	cmp r4, r10	;11 or 00
	BEQ b_cmp_neg	; cmp negative number
b_cmp_pos
	;2. exponent cmp
	and r4, r2, r11 ; r2's exponent bit
	and r5, r3, r11 ; r3's exponent bit
	cmp r4, r5		;cmp exponent bit
	BHI b_if_grater_r2 ; r4 > r5
	BMI b_if_grater_r3 ; r5 > r4
...
b_cmp_neg
	...
	and r5, r3, r11 ; r3's exponent bit
	cmp r4, r5		;cmp exponent bit
	BHI b_if_grater_r3 ; |r4| > |r5|
	BMI b_if_grater_r2 ; |r5| > |r4|
...
b_if_grater_r3 ;or same  
	;no change
	add r0, r0, #4 ;next element address
	b b_loop_cmp ;next cmp

b_end_cmp
	sub r7, #4 ;last element sorted
	cmp r7, r8 ;finish sort
	BXEQ lr
	B b_loop_sort
...
selection_sort
	...
	ldr r0, =dataset ; r0 = address of a element to sort
s_loop_sort
	mov r7, r0 			;r7 = least data address	
	add r1, r7, #4		;address to cmp
	cmp r1, r6		;check end
	BXEQ lr			;end
s_loop_cmp
	cmp r1, r6		;check end
	BEQ s_end_cmp		;end
	ldr r3, [r7]	; r7 least data
	ldr r2, [r1]	; r1 data
	...
	and r5, r3, r10 ; r3's sign bit
	cmp r4, r5		;cmp sign bit (0 or 1)
	BHI s_if_grater_r3 ; 10 1-0 = 1
	BMI s_if_grater_r2 ; 01 0-1 = -1
	
	cmp r4, r10	;11 or 00
	BEQ s_cmp_neg	; cmp negative number
s_cmp_pos
	;2. exponent cmp
	and r4, r2, r11 ; r2's exponent bit
	and r5, r3, r11 ; r3's exponent bit
	cmp r4, r5		;cmp exponent bit
	BHI s_if_grater_r2 ; r4 > r5
	BMI s_if_grater_r3 ; r5 > r4
...
s_cmp_neg
	...
	and r5, r3, r11 ; r3's exponent bit
	cmp r4, r5		;cmp exponent bit
	BHI s_if_grater_r3 ; |r4| > |r5|
	BMI s_if_grater_r2 ; |r5| > |r4|
	...
	cmp r4, r5		;cmp mantissa bit
	BHI s_if_grater_r3 ; |r4| > |r5|
	BLS s_if_grater_r2 ; |r5| >= |r4|
	
s_if_grater_r2
	add r1, r1, #4 ;next address
	b s_loop_cmp ;next cmp

s_if_grater_r3 ;or same
	;r3[r7] <- r2[r1]
	mov r3, r2
	mov r7, r1
	add r1, r1, #4 ;next address
